...
CURRENT_DIAGRAM = Diagram(
    ...
    subtitle=(
        "First-release pipeline: deterministic synthetic data, star Bayesian "
        "network, fixed 70/30 fusion."
    ),
    nodes=(
        Node("A", "Deterministic synthetic tables", 0, 0, "data", col_span=2),
        Node("B", "Validation", 0, 1, "data", col_span=2),
        Node("C", "Retrospective OTIF + 7-cause rules", 0, 2, "data", col_span=2),
        Node("D", "Point-in-time order features", 0, 3, "data", col_span=2),
        Node("E", "Chronological 60/20/20 split", 0, 4, "data", col_span=2),
        Node("F", "XGBoost + calibration + SHAP", 0, 5, "model"),
        Node("G", "7 causes direct to OTIF (star BN)", 1, 5, "model"),
        Node("H", "70/30 score fusion", 0, 6, "decision", col_span=2),
        Node("I", "Fused threshold", 0, 7, "decision", col_span=2),
        Node("J", "Mitigations + conflict checks + impact", 0, 8, "decision", col_span=2),
        Node("K", "Order queue + rollups + Streamlit", 0, 9, "product", col_span=2),
        Node("L", "Append-only planner feedback", 0, 10, "product", col_span=2),
    ),
    edges=(
        Edge("A", "B"),
        Edge("B", "C"),
        ...
        Edge("F", "H", "model"),
        Edge("G", "H", "model"),
        Edge("H", "I", "intervention"),
        Edge("I", "J", "intervention"),
        Edge("J", "K", "intervention"),
        Edge("K", "L"),
    ),
)
...
TARGET_DIAGRAM = Diagram(
    ...
    subtitle=(
        "Noisy digital twin, compact causal chain, evidence-based fusion, "
    ...
    nodes=(
        ...
        Node("F1", "Compact causal Bayesian chain", 2.4, 6, "model"),
        ...
        Node("K3", "Model health + causal pathway view", 2.6, 12, "product"),
